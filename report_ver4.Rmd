...
```{r}  
  ...
  for (i in 1:nbfacs)
  {
    ...
    MAplot(raw(),which=i,plotFun=smoothScatter,refSamples=c(1:nbfacs),main=' versus median of all samples')
...
#  MAplot(norm(),pairs=TRUE,plotFun=smoothScatter,main="MVA plot after RMA Normalization", labels=norm()$SampleID) 
  ...
  for (i in 1:nbfacs2)
  {
    ...
    MAplot(norm(),which=i,plotFun=smoothScatter,refSamples=c(1:nbfacs2),main=' versus median of other samples')
  }
``` 
...
#---------
#3D PCA code using shiny language
...
gene=all.genes.con$SYMBOL
volcano_data=as.data.frame(cbind(gene,log_FC,log_pval,Significant))
k = plot_ly(data = volcano_data, x = log_FC, y = log_pval, text = gene, mode = "markers", color = Significant) %>% layout(title =paste("Volcano Plot for Pre Treatment vs Baseline"),xaxis=list(title="Fold Change",range =c(-4,4),tickvals=c(-4,-3,-2,-1,0,1,2,3,4),ticktext=c('-16','-8','-4','-2','1','2','4','8','16')),yaxis=list(title="-Log10 pvalue",range =c(0,15)))
